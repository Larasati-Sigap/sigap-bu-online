...
async function loadUsers(){
  ...
  return <div className="app"><aside className="sidebar"><div className="brand"><div className="brandLogo">⚖</div><div><strong>SIGAP-BU</strong><span>Online v3.0</span></div></div><div className="userCard"><div className="avatar">{initials(profile?.nama||session.user.email)}</div><div><strong>{profile?.nama||session.user.email}</strong><span>{profile?.role||'viewer'} · {profile?.unit_kerja||'Semua Unit'}</span></div></div><p className="navTitle">NAVIGATION</p>{[['dashboard',Activity,'Dashboard'],['pegawai',Users,'Data Pegawai'],['update',RotateCcw,'Update Pegawai'],['riwayat',ClipboardList,'Audit Trail'],['backup', Database, 'Backup'],['users', Users, 'Manajemen User']].map(([key,Icon,label])=><button key={key} className={`navItem ${active===key?'active':''}`} onClick={()=>setActive(key)}><Icon size={18}/>{label}</button>)}<button className="btn outline logout" onClick={()=>supabase.auth.signOut()}><LogOut size={16}/>Keluar</button></aside><main className="main"><header className="hero"><div><p className="breadcrumb">SIGAP-BU › {active}</p><h2>{active==='dashboard'?'Dashboard SIGAP-BU':active==='pegawai'?'Data Pegawai':active==='update'?'Update Pegawai':active==='riwayat'?'Audit Trail':active==='users'?'Manajemen User':'Backup & Export'}</h2><p>Data tersimpan di Supabase dan sinkron untuk seluruh user.</p></div>{active!=='users'&&<div className="heroActions"><button className="btn glass" onClick={exportCsv}><Download size={16}/>Export CSV</button>{canEdit&&<button className="btn gold" onClick={openNew}><Plus size={16}/>Tambah Pegawai</button></div>}
