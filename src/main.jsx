...
const pensiunSatuTahun = pegawaiAktif
 ...
 async function addHistory(pegawai_id,tipe,field,oldValue,newValue,catatan,pegawaiNama=''){
  ...
  if(quick.tipe === 'Naik Pangkat'){
    if(!quick.jenis) return alert('Pilih jenis pegawai terlebih dahulu.')
    if(!quick.pangkat_gol) return alert('Pilih pangkat/golongan baru.')

    payload.pangkat_gol = quick.pangkat_gol
    payload.jenis = quick.jenis
    payload.tmt_pangkat = quick.tanggal || null
    field = 'pangkat_gol'
    old = p.pangkat_gol
    neu = quick.pangkat_gol
  }

  if(quick.tipe === 'Mutasi'){
    const tujuanMutasi = quick.unit_kerja === '__MANUAL__'
      ? quick.unit_manual
      : quick.unit_kerja

    if(!tujuanMutasi) return alert('Pilih/isi unit kerja tujuan mutasi.')

    payload.unit_kerja = tujuanMutasi
  ...
  if(quick.tipe === 'Pensiun'){
    payload.tanggal_pensiun = quick.tanggal || null
    payload.status = 'Pensiun'
  }
  ...
  await addHistory(p.id,quick.tipe,field,old,neu,quick.catatan,p.nama)
